    ...
    for (const ing of ingRows) {
      ...
      if (actId) {
        await pool.query(
          ...
          [actId, ing.ingredient_id, dateAppro, quantiteConsumed, ing.last_prix || 0, 'TP', autoFournisseurId, `${ing.nom}-${yearStr}`]
        );
      } else {
        await pool.query(
          `INSERT INTO stock_client_daily (client_id, ingredient_id, date_appro, quantite, prix_unitaire, type_appro, fournisseur_id, ref_facture)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
          [userId, ing.ingredient_id, dateAppro, quantiteConsumed, ing.last_prix || 0, 'TP', autoFournisseurId, `${ing.nom}-${yearStr}`]
        );
      }
    }
